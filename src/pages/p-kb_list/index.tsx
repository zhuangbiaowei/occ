...
const mapKnowledgeBase = (kb: KnowledgeBaseResponse): KnowledgeBase => ({
  id: kb.id,
  name: kb.name,
  description: kb.description || '',
  documentCount: kb.documentCount ?? kb.documents?.length ?? 0,
  documentNewThisMonth: kb.documentNewThisMonth ?? 0,
  creator: kb.createdBy?.fullName || kb.createdBy?.username || '系统',
...
const KnowledgeBaseListPage: React.FC = () => {
  ...
  const loadKnowledgeBases = async () => {
    ...
    try {
      ...
      setKnowledgeBases(items.map(mapKnowledgeBase));
      setSummary({
        totalKnowledge: data.totalKnowledge ?? data.total ?? items.length,
        totalDocument: data.totalDocument ?? 0,
        newDocumentsThisMonth: data.newDocumentsThisMonth ?? 0,
      });
    } catch (error) {
      setLoadError('Failed to load knowledge bases');
      setKnowledgeBases([]);
      setSummary({
        totalKnowledge: 0,
        totalDocument: 0,
  ...
  // 处理新建知识库
  const handleNewKnowledgeSubmit = async () => {
    ...
    try {
      ...
      setKnowledgeBases(prev => [mapKnowledgeBase(created), ...prev]);
      setShowNewKnowledgeModal(false);
      setNewKnowledgeForm({ name: '', description: '' });
    } catch (error) {
      setLoadError('֪ʶ�ⴴ��ʧ��');
  ...
  const handleDeleteKnowledge = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/knowledge-bases/${deleteKnowledgeInfo.id}`,
        {
          method: 'DELETE',
          headers: buildAuthHeaders(accessToken),
        }
      ...
      setKnowledgeBases(prev =>
        prev.filter(kb => kb.id !== deleteKnowledgeInfo.id)
      );
      setShowDeleteConfirmModal(false);
      setDeleteKnowledgeInfo(null);
    } catch (error) {
      setLoadError('Failed to delete knowledge base');
    }
  };
  ...
  return (
    <div className={styles.pageWrapper}>
      ...
      <main className="ml-64 mt-16 p-6 min-h-screen">
        {isLoading && (
          <div className="mb-4 rounded-lg bg-blue-50 px-4 py-3 text-sm text-text-secondary">
            正在加载知识库...
          </div>
        )}
        {loadError && (
        ...
        <section className="bg-white rounded-xl shadow-card p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-3xl font-bold text-primary mb-2">{summary.totalKnowledge}</div>
              <div className="text-sm text-text-secondary">总知识库</div>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-3xl font-bold text-success mb-2">
                {summary.totalDocument}
              </div>
              <div className="text-sm text-text-secondary">文档总数</div>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-3xl font-bold text-warning mb-2">
                {countNewThisMonth(summary.newDocumentsThisMonth)}
              </div>
        ...
        <section className="bg-white rounded-xl shadow-card">
                  ...
                  <tr key={knowledgeBase.id} className={styles.tableRow}>
                      ...
                      <div className="flex items-center">
                        <div
                          className={`w-10 h-10 rounded-lg flex items-center justify-center mr-3 ${
                            knowledgeBase.iconColor === 'text-primary'
                              ? 'bg-blue-100'
                              : knowledgeBase.iconColor === 'text-success'
                                ? 'bg-green-100'
                                : 'bg-orange-100'
                          }`}
                        >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-text-primary">
                        {knowledgeBase.documentCount.toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="px-6 py-4 border-t border-border-light flex items-center justify-between">
            <div className="text-sm text-text-secondary">
              显示 1-{filteredAndSortedKnowledgeBases.length} 条，共{' '}
              {filteredAndSortedKnowledgeBases.length} 条记录
            </div>
            <div className="flex items-center space-x-2">
              <button
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e =>
                        setNewKnowledgeForm(prev => ({ ...prev, name: e.target.value }))
                      }
                      required
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={e =>
                        setNewKnowledgeForm(prev => ({ ...prev, description: e.target.value }))
                      }
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
              <div className="p-6 text-center">
                ...
                <p className="text-sm text-text-secondary mb-6">
                  {'确定要删除知识库 "'}
                  <span>{deleteKnowledgeInfo.name}</span>
                  {'" 吗？此操作不可撤销，将删除该知识库中的所有文档。'}
                </p>
