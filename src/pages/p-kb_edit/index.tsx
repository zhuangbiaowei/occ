...
const KbEditPage: React.FC = () => {
  ...
  const mockDocuments: Record<string, DocumentData> = {
    'doc1': {
      title: '开源许可证合规指南',
      content: '<h1>开源许可证合规指南</h1><p>本指南详细介绍了常见的开源许可证及其合规要求。</p><h2>1. MIT许可证</h2><p>MIT许可证是最宽松的开源许可证之一，允许商业使用、修改和分发。</p><ul><li>必须保留原始版权声明</li><li>可以用于商业产品</li><li>无需公开修改后的源代码</li></ul>',
      tags: ['开源许可证', 'MIT', '合规指南'],
      knowledgeBase: '法律知识库',
    ...
    'doc2': {
      title: 'GPL许可证详解',
      content: '<h1>GPL许可证详解</h1><p>GPL（GNU General Public License）是一种 copyleft 许可证，要求衍生作品也必须使用GPL许可证。</p><h2>主要条款</h2><p>GPL的核心要求是：</p><ol><li>必须提供源代码</li><li>修改后的作品必须同样开源</li><li>必须保留原始许可证</li></ol>',
  ...
  useEffect(() => {
    ...
    if (contentEditor && !documentData.content.trim()) {
      contentEditor.innerHTML = '<div class="text-text-secondary placeholder-text-secondary">请输入文档内容...</div>';
    }
  ...
  const handleEditorFocus = () => {
    const contentEditor = contentEditorRef.current;
    if (contentEditor && contentEditor.innerHTML.trim() === '<div class="text-text-secondary placeholder-text-secondary">请输入文档内容...</div>') {
  ...
  const getEditorContent = (): string => {
    ...
    if (content === '<div class="text-text-secondary placeholder-text-secondary">请输入文档内容...</div>') {
      return '';
    }
    return content;
  };
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
            <form onSubmit={handleFormSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label htmlFor="document-title" className="block text-sm font-medium text-text-primary">
              ...
              {/* 文档内容编辑器 */}
              <div className="space-y-2">
                <label htmlFor="document-content" className="block text-sm font-medium text-text-primary">
                  文档内容 <span className="text-danger">*</span>
                </label>
                
                {/* 编辑器工具栏 */}
                <div className={`${styles.editorToolbar} flex flex-wrap items-center gap-1 p-3 border border-border-light rounded-t-lg bg-gray-50`}>
                  <button 
                    type="button" 
                ...
                <p className="text-xs text-text-secondary">支持富文本编辑，可使用工具栏添加格式、列表、链接等</p>
              </div>
