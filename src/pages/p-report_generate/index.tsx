...
const ReportGeneratePage: React.FC = () => {
  ...
  const [reviewData, setReviewData] = useState<ReviewData>({
    ...
    summary: '本次评审针对SBOM文件"project-x-v2.1.spdx"进行了全面的合规性分析。共识别开源组件23个，其中高风险组件2个，中风险组件5个，低风险组件16个。主要风险点包括许可证兼容性问题和专利风险。'
  });
  ...
  useEffect(() => {
    const mockReviewData: Record<string, ReviewData> = {
      ...
      'review-002': {
        ...
        summary: '本次评审针对SBOM文件"mobile-app-v1.5.cdx"进行了全面的合规性分析。共识别开源组件18个，其中高风险组件1个，中风险组件3个，低风险组件14个。主要风险点为许可证兼容性问题。'
      }
    };
        ...
        <div className={`bg-white rounded-xl shadow-modal w-full max-w-2xl max-h-[90vh] overflow-y-auto ${styles.modalEnter}`}>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label htmlFor="report-name" className="block text-sm font-medium text-text-primary">报告名称</label>
                <input 
                  type="text" 
                  id="report-name" 
                  name="report-name"
              ...
              {/* 报告模板选择 */}
              <div className="space-y-2">
                <label htmlFor="template-select" className="block text-sm font-medium text-text-primary">报告模板</label>
                <select 
                  id="template-select" 
          ...
          <div className="flex items-center justify-end space-x-3 p-6 border-t border-border-light bg-bg-light rounded-b-xl">
            ...
            >
              ...
              <i className={`fas ${isGenerating ? 'fa-spinner' : 'fa-download'} ${isGenerating ? styles.loadingSpinner : ''}`}></i>
            </button>
          </div>
        </div>
      </div>
      ...
      {showLoadingOverlay && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-60">
          <div className="bg-white rounded-lg p-6 flex items-center space-x-4">
            <div className={`${styles.loadingSpinner} w-6 h-6 border-2 border-primary border-t-transparent rounded-full`}></div>
