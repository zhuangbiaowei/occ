...
const SysSettingsPage: React.FC = () => {
  ...
  const filteredLogs = logEntries.filter(log => {
    const matchesSearch = log.detail.toLowerCase().includes(logSearchTerm.toLowerCase()) ||
                         log.user.toLowerCase().includes(logSearchTerm.toLowerCase());
    const matchesType = !logTypeFilter || log.type.toLowerCase().includes(logTypeFilter.toLowerCase());
    ...
    <div className={styles.pageWrapper}>
      ...
      <main className="ml-64 mt-16 p-6 min-h-screen">
        ...
        <div className="space-y-6">
          {/* 基本配置 */}
          <div className={`${styles.configSection} ${expandedSections.basic ? styles.expanded : styles.collapsed} bg-white rounded-xl shadow-card overflow-hidden`}>
            <div 
            ...
            <div className={`${styles.configContent} px-6 pb-6`}>
              <form onSubmit={handleBasicConfigSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="system-name" className="block text-sm font-medium text-text-primary mb-2">系统名称</label>
                    <input 
                      type="text" 
                      id="system-name" 
                      value={basicConfig.systemName}
                      onChange={(e) => setBasicConfig(prev => ({ ...prev, systemName: e.target.value }))}
                  ...
                  <div>
                    <label htmlFor="system-version" className="block text-sm font-medium text-text-primary mb-2">系统版本</label>
                    <input 
                      type="text" 
                      id="system-version" 
                      value={basicConfig.systemVersion}
                      onChange={(e) => setBasicConfig(prev => ({ ...prev, systemVersion: e.target.value }))}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="company-name" className="block text-sm font-medium text-text-primary mb-2">公司名称</label>
                    <input 
                      type="text" 
                      id="company-name" 
                      value={basicConfig.companyName}
                      onChange={(e) => setBasicConfig(prev => ({ ...prev, companyName: e.target.value }))}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    />
                  </div>
                  <div>
                    <label htmlFor="contact-email" className="block text-sm font-medium text-text-primary mb-2">联系邮箱</label>
                    <input 
                      type="email" 
                      id="contact-email" 
                      value={basicConfig.contactEmail}
                      onChange={(e) => setBasicConfig(prev => ({ ...prev, contactEmail: e.target.value }))}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    />
                  </div>
                </div>
          ...
          {/* 通知设置 */}
          <div className={`${styles.configSection} ${expandedSections.notification ? styles.expanded : styles.collapsed} bg-white rounded-xl shadow-card overflow-hidden`}>
            <div 
              className="flex items-center justify-between p-6 cursor-pointer"
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-3">
                    <input 
                      ...
                      onChange={(e) => setNotificationConfig(prev => ({ ...prev, emailNotification: e.target.checked }))}
                      className="w-4 h-4 text-primary border-border-light rounded focus:ring-primary"
                    />
                    <label htmlFor="email-notification" className="text-sm font-medium text-text-primary">邮件通知</label>
                  </div>
                  <div className="flex items-center space-x-3">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => setNotificationConfig(prev => ({ ...prev, systemNotification: e.target.checked }))}
                      className="w-4 h-4 text-primary border-border-light rounded focus:ring-primary"
                    />
                    <label htmlFor="system-notification" className="text-sm font-medium text-text-primary">系统内通知</label>
                  </div>
                  <div className="flex items-center space-x-3">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => setNotificationConfig(prev => ({ ...prev, reviewCompletedNotification: e.target.checked }))}
                      className="w-4 h-4 text-primary border-border-light rounded focus:ring-primary"
                    />
                    <label htmlFor="review-completed-notification" className="text-sm font-medium text-text-primary">评审完成通知</label>
                  </div>
                  <div className="flex items-center space-x-3">
                    <input 
                      type="checkbox" 
                      ...
                      onChange={(e) => setNotificationConfig(prev => ({ ...prev, kbUpdateNotification: e.target.checked }))}
                      className="w-4 h-4 text-primary border-border-light rounded focus:ring-primary"
                    />
                    <label htmlFor="kb-update-notification" className="text-sm font-medium text-text-primary">知识库更新通知</label>
                  </div>
                </div>
                <div className="flex justify-end">
                  <button 
          ...
          {/* 数据备份 */}
          <div className={`${styles.configSection} ${expandedSections.backup ? styles.expanded : styles.collapsed} bg-white rounded-xl shadow-card overflow-hidden`}>
            <div 
              className="flex items-center justify-between p-6 cursor-pointer"
            ...
            <div className={`${styles.configContent} px-6 pb-6`}>
              <form onSubmit={handleBackupConfigSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="backup-frequency" className="block text-sm font-medium text-text-primary mb-2">备份频率</label>
                    <select 
                      id="backup-frequency" 
                      value={backupConfig.backupFrequency}
                      onChange={(e) => setBackupConfig(prev => ({ ...prev, backupFrequency: e.target.value }))}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    >
                  ...
                  <div>
                    <label htmlFor="backup-retention" className="block text-sm font-medium text-text-primary mb-2">备份保留</label>
                    <select 
                      id="backup-retention" 
                      value={backupConfig.backupRetention}
                      onChange={(e) => setBackupConfig(prev => ({ ...prev, backupRetention: e.target.value }))}
                      className="w-full px-4 py-2 border border-border-light rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                    >
                ...
                <div className="flex items-center space-x-3">
                  <input 
                    ...
                    className="w-4 h-4 text-primary border-border-light rounded focus:ring-primary"
                  />
                  <label htmlFor="auto-backup" className="text-sm font-medium text-text-primary">启用自动备份</label>
                </div>
          ...
          {/* 操作日志 */}
          <div className={`${styles.configSection} ${expandedSections.logs ? styles.expanded : styles.collapsed} bg-white rounded-xl shadow-card overflow-hidden`}>
            <div 
              className="flex items-center justify-between p-6 cursor-pointer"
            ...
            <div className={`${styles.configContent} px-6 pb-6`}>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border-light">
                      <th className="text-left py-3 px-4 text-sm font-medium text-text-secondary">操作时间</th>
                      <th className="text-left py-3 px-4 text-sm font-medium text-text-secondary">用户</th>
                      <th className="text-left py-3 px-4 text-sm font-medium text-text-secondary">操作类型</th>
                      <th className="text-left py-3 px-4 text-sm font-medium text-text-secondary">操作详情</th>
                      <th className="text-left py-3 px-4 text-sm font-medium text-text-secondary">IP地址</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center space-x-2">
                  <button className="px-3 py-1 border border-border-light rounded hover:bg-gray-50">上一页</button>
                  <button className="px-3 py-1 bg-primary text-white rounded">1</button>
                  <button className="px-3 py-1 border border-border-light rounded hover:bg-gray-50">2</button>
                  <button className="px-3 py-1 border border-border-light rounded hover:bg-gray-50">3</button>
                  <button className="px-3 py-1 border border-border-light rounded hover:bg-gray-50">下一页</button>
                </div>
              </div>
            </div>
          </div>
